...
# 
# Usage: advisory-creator.sh --current-tag <tag> [--previous-tag <tag>] [--dry-run] [--debug]
#
# Arguments:
...
#
# Example:
#   advisory-detection.sh --current-tag v3.2.1
#   advisory-detection.sh --current-tag v3.2.1 --previous-tag v3.2.0 --dry-run
...
Example:
  $(basename "${0}") --current-tag v3.2.1
  $(basename "${0}") --current-tag v3.2.1 --previous-tag v3.2.0 --dry-run
EOF
}
...
function create_pull_request() {
  ...
  if ! gh pr create --title "${pr_title}" --body "${pr_body}" --repo "$(git remote get-url origin | sed 's/.*github.com[:/]\(.*\).git/\1/')"; then
