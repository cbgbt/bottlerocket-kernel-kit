...
function generate_advisory_file() {
  ...
  # shellcheck disable=SC2016 # We want to pass the variable names to envsubst
...
# Parse file info string into components
#
# Args:
#   $1: File info string in format "advisory_id:package_name:version:content"
#
# Returns:
#   Advisory ID, package name, version, and content as separate lines
function parse_file_info() {
  local file_info="$1"
  
  # Extract components
  local advisory_id package_name version content
  advisory_id=$(echo "${file_info}" | cut -d':' -f1)
  package_name=$(echo "${file_info}" | cut -d':' -f2)
  version=$(echo "${file_info}" | cut -d':' -f3)
  content=$(echo "${file_info}" | cut -d':' -f4-)
...
# Format PR body with kernel updates
#
# Args:
#   $1: Kit version
#   $2: Previous version
#   $3: Kernel updates string with newlines
#   $4: Optional template path
#
# Returns:
#   Formatted PR body
function format_pr_body() {
  ...
  local kernel_updates_str="$3"
  local template_path="${4:-$PR_BODY_TEMPLATE}"
  ...
  local result
  # shellcheck disable=SC2016 # We want to pass the variable names to envsubst
  result=$(echo -e "${template}" | envsubst '${KIT_VERSION} ${PREVIOUS_VERSION} ${KERNEL_UPDATES}')
...
function create_pull_request() {
  ...
  for file_info in "${advisory_files[@]}"; do
    # Parse file info using the parse_file_info function
    local components
    mapfile -t components < <(parse_file_info "${file_info}")
    local advisory_id="${components[0]}"
    local package_name="${components[1]}"
    local version="${components[2]}"
    local content="${components[3]}"
  ...
  # Process PR title template
  export KIT_VERSION="${kit_version#v}"
  # shellcheck disable=SC2016 # We want to pass the variable name to envsubst
  local pr_title
  ...
  local pr_body
  pr_body=$(format_pr_body "${kit_version}" "${previous_version}" "${kernel_updates_str}")
